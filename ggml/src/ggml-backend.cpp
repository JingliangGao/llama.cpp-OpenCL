...
#include "ggml-profiler.h"    /* insert profiler header     JingliangGao 2026/06/22 */
#include <string>
#endif
...
#ifdef GGML_UNIFY_PROFILER
    // Clean up profiler if present (before backend frees its context)      JingliangGao 2026/06/22
...
#if defined(GGML_UNIFY_PROFILER)
/* Build a COPY profiling record.     JingliangGao 2026/06/22 */
// Copies have no real ggml_tensor "node" backing them, so we synthesize one source describing the input tensor that was moved.
static ggml_profile_record make_copy_record(const char * copy_flag, int backend_id, int split_id,
...
#if defined(GGML_UNIFY_PROFILER)
    /* reset copy records for this compute pass     JingliangGao 2026/06/22 */
...
#if defined(GGML_UNIFY_PROFILER)
        /* set split ID and enable backend profiling     JingliangGao 2026/06/22 */
        if (sched->profiling_enabled) {
            ...
            if (input->flags & GGML_TENSOR_FLAG_INPUT) {
...
#if defined(GGML_UNIFY_PROFILER)
                /* record copy node event    JingliangGao 2026/06/22 */
                if (sched->profiling_enabled) {
                    ...
                    sched->copy_records.push_back(make_copy_record(copy_flag, split_backend_id, split_id,
                ...
                if (split->graph.n_nodes > 0 &&
...
#if defined(GGML_UNIFY_PROFILER)
                    size_t total_copied_bytes = 0;    /*  initialize total number of bytes copied     JingliangGao 2026/06/22 */
#endif
...
#if defined(GGML_UNIFY_PROFILER)
                        total_copied_bytes += expert_size_copy + padding_end;    /* record total number of bytes copied     JingliangGao 2026/06/22 */
#endif
...
#if defined(GGML_UNIFY_PROFILER)
                    /* record moe copy start time    JingliangGao 2026/06/22 */
...
#if defined(GGML_UNIFY_PROFILER)
                    /* record moe copy event  JingliangGao 2026/06/22 */
                    if (sched->profiling_enabled) {
...
#if defined(GGML_UNIFY_PROFILER)
                    /* Capture timestamp before async attempt so we can record launch time     JingliangGao 2026/06/22 */
                    ...
                    if (!split_backend->iface.cpy_tensor_async || !split_backend->iface.cpy_tensor_async(input_backend, split_backend, input, input_cpy)) {
...
#else
                        if (sched->profiling_enabled) {
                            /* sync copy was launched,  record the time spanning the async call    JingliangGao 2026/06/22 */
                            copy_start = ggml_profiler_time_ns();
                            ggml_backend_tensor_copy(input, input_cpy);
                            uint64_t copy_end = ggml_profiler_time_ns();
                        ...
                        /* async copy was launched,  record the time spanning the async call    JingliangGao 2026/06/22 */
                        if (sched->profiling_enabled) {
                            uint64_t copy_end = ggml_profiler_time_ns();
...
#if defined(GGML_UNIFY_PROFILER)
    if (sched->profiling_enabled) {
        /* Collect backend operation records    JingliangGao 2026/06/22 */
        for (int b = 0; b < sched->n_backends; b++) {
...
#if defined(GGML_UNIFY_PROFILER)
    /* read profiling environment variable     JingliangGao 2026/06/23  */
    const char * profile_env = getenv("GGML_UNIFY_PROFILE");
    if (profile_env != NULL) {
...
#if defined(GGML_UNIFY_PROFILER)
    /* Auto-export profiling data if enabled via GGML_PROFILE env var    JingliangGao 2026/06/23 */
